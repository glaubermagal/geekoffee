...
description = "In this article, we will explain a few well known Cryptographic Primitives that ensure integrity, authenticity, and non-repudiation in data transmission using node.js"
...
- **Availability:** is the information available when it is needed? (This concept will not be covered by this article.)
...
A Message Authentication Code (MAC) is a short piece of information used to authenticate a message. In other words, it’s used to confirm that the message came from the expected sender and has not been changed without your knowledge. The MAC value ensures both the **integrity** and **authenticity** of a message, by regenerating it in the recipient using a shared secret **key (K)**.
...
HMACs are similar to digital signatures. Both enforce **integrity** and **authenticity**, also both use cryptographic keys and  hash functions. The main difference is that digital signatures use asymmetric keys, while HMACs use symmetric keys.
...
Although, just comparing the two hashes as strings would be enough, in order to avoid timing attacks you should compare the time that hashes were generated. For more information about this sort of issue, see [Coda Hale’s blog post](https://codahale.com/a-lesson-in-timing-attacks/) about the timing attacks on KeyCzar and Java’s `MessageDigest.isEqual()`.

Suppose a client is sending a message to a server, but the message or the hash was modified during its transmission, configuring a man-in-the-middle attack. In this case, the server should detect the possible attack and reject the request.
...
Digital Signatures work with asymmetric key-pairs, one private and others public. In the example below, we will be generating the keys with [OpenSSL](https://github.com/openssl/openssl).
...
You can take a look at these codes on [my github repo](https://github.com/glaubermagal/digital-signature-example).


## What Cryptographic Primitive should you use?

This depends on the context. Sometimes you will communicate between only two servers on a private network, so a simple HMAC validation might be enough. Sometimes your system will be shared by an unknown number of clients and servers, so in this case, you might be interested in enforcing **non-repudiation**, instead of just sharing the secret key.

## Conclusion

Hopefully, the above explanation will help you implement some Security Information best practices in your systems using node.js. Additionally, you should be aware that there are many kinds of advanced attacks that should be prevented by using other security layers and strategies.
